...
1. **Modelo de precio por plataforma (BLOQUEA Fase 1).** Cuando un cliente viene por Groupon/Smartbox: ¿paga un precio de venta distinto al directo (la plataforma empaqueta su oferta) y iJump recibe ese importe menos comisión, o el precio de catálogo es el mismo en todas partes y solo cambia el % que retiene la plataforma? Son dos modelos de datos distintos. *(Ricardo debe responder antes de itemizar.)*
