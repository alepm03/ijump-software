...
- **Disponibilidad a nivel de día, no de franja horaria.** El motor clasifica por capacidad total del día (`max_flights_per_day` × `max_clients_per_flight`), no por conflicto de horario exacto. Decisión explícita: el staff puede crear dos vuelos a la misma hora si hace falta; el sistema no lo impide ni lo avisa. Deferido a una fase futura si se pide.
...
- **`reservations_assign_seat` nunca reasigna a un vuelo de otra hora** solo porque tenga hueco — solo une al vuelo de la hora exacta solicitada o crea uno nuevo (en esa hora, o en la siguiente libre). Esta regla viene de una corrección explícita tras pruebas reales; no revertirla sin que el usuario lo pida.
